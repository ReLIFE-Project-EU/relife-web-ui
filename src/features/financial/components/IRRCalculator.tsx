...
import type { IRRRequest, IRRResponse } from "../../../types/financial";
...
export const IRRCalculator = () => {
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      const projectLifetimeNum = Number(projectLifetime);
      ...
      const request: IRRRequest = {
        ...
        project_lifetime: projectLifetimeNum,
        energy_mix: energyMix,
        energy_prices: energyPrices,
      };

      const response = await financial.calculateIRR(request);
      setResult(response);
    } catch (err) {
      console.error(err);
      setError(
