...
**Contract truth**: Confirm behavior in the service codebase under `external-services/` (handlers, models, validation), or in the upstream GitHub repository when those clones are not present. Generated OpenAPI from a running service is not sufficient on its own—it is often incomplete or stale relative to handlers and validation.
...
Before touching any code, collect the following from the user using the active
agent's normal user-input mechanism:
...
Use the active agent's normal user-input mechanism for mode selection; gather the
rest as free-form input.
...
- **Agent-assisted** — delegate the implementation to an available coding agent (recommended for
...
Delegate the implementation to the appropriate available coding agent. Use a
lower-latency worker for simple isolated changes and a stronger/deeper worker for
complex architectural changes, following the host agent's delegation conventions.
